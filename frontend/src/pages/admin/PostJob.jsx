...
const PostJob = () => {
    ...
    const submitHandler = async (e) => {
        ...
        if(!input.requirements?.trim()) return toast.error("Requirements are required");
        
        if(input.salaryType !== 'Not Disclosed') {
            if(input.salaryMin === "" || Number(input.salaryMin) < 1) return toast.error("Salary amount is required and must be at least 1");
        ...
        if(locations.length === 0) return toast.error("Please add at least one location");
        if(!input.employmentType) return toast.error("Employment Type is required");
        if(!input.workMode) return toast.error("Work Mode is required");
        if(!input.jobType) return toast.error("Job Type is required");
        if(!input.experienceLevel) return toast.error("Experience Level is required");
        if(input.position === "" || Number(input.position) < 1) return toast.error("Number of Positions must be at least 1");
        if(companies.length === 0) return toast.error("Please register a company first before posting a job");
    ...
    return (
        <div className="bg-[#EEF1F5] min-h-screen">
            ...
            <div className='max-w-4xl mx-auto my-10 px-4'>
                <div className='bg-white border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-2xl p-6 sm:p-8 relative overflow-hidden'>
                    ...
                    <div className='flex items-center gap-5 mb-8 pb-8 border-b border-slate-100 relative z-10'>
                        <Button onClick={() => navigate(-1)} variant="outline" className="flex items-center gap-2 text-slate-600 font-semibold border-slate-200 hover:bg-slate-50">
                            <ArrowLeft className="w-4 h-4"/>
                            <span>Back</span>
                        </Button>
                        <div>
                            <h1 className='font-bold text-2xl text-slate-900'>Post a New Job</h1>
                            <p className='text-slate-500 mt-1'>Fill in the details below to create a new job listing.</p>
                        </div>
                    </div>
                    <form onSubmit={submitHandler} className="relative z-10 space-y-10">
                        ...
                        <div className="pt-8 border-t border-slate-100">
                            ...
                            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                                ...
                                <div>
                                    ...
                                    <div className="mt-2">
                                        <Select onValueChange={(value) => setInput({...input, experienceLevel: value})}>
                                            <SelectTrigger className="w-full py-6 bg-slate-50 border-slate-200">
